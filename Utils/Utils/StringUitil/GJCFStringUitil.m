...
+ (BOOL)stringIsAllChineseWordAndCharacter:(NSString *)string
{
    NSString *checkRegex = @"^([A-Za-z]|[\u4E00-\u9FA5])+$";
    return [GJCFStringUitil sourceString:string regexMatch:checkRegex];
}
...
+ (NSString *)urlEncode:(id)object {
    ...
    CFStringRef aCFString = CFURLCreateStringByAddingPercentEscapes(nil, (CFStringRef)string, nil, (CFStringRef)@"!*'();:@&=+$,/?%#[]", kCFStringEncodingUTF8);
    NSString *encodedValue = [NSString stringWithString:(__bridge NSString*)aCFString];
    CFRelease(aCFString);
    return encodedValue;
